...
/**
 * Install console command - Installs Kiboko CI.
...
class InstallCommand extends Command
{
    ...
    {
        $defaultPath = KIBOKO_CI_APP_DIR . 'app/config.yml';
    ...
    /**
     * Installs Kiboko CI - Can be run more than once as long as you ^C instead of entering an email address.
     */
    ...
    /**
     * Load configuration for Kiboko CI form CLI options or ask info to user.
     *
     * @param InputInterface $input
     * @param OutputInterface $output
     * @return array
    ...
    protected function verifyDatabaseDetails(array $db, OutputInterface $output)
    {
        try {
            $pdo = new PDO(
                'mysql:host='.$db['servers']['write'].';port='.$db['servers']['write_port'].';dbname='.$db['name'],
                $db['username'],
                $db['password'],
                array(
    ...
    /**
     * Write the Kiboko CI config.yml file.
     * @param array $config
     */
    ...
    protected function setupDatabase(OutputInterface $output)
    {
        ...
        $phinxBinary = escapeshellarg(KIBOKO_CI_APP_DIR . 'vendor/bin/phinx');
        $phinxScript = escapeshellarg(KIBOKO_CI_APP_DIR . 'phinx.php');
