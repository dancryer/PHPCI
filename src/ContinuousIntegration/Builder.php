...
use Kiboko\Component\ContinuousIntegration\Plugin\Util\Factory as PluginFactory;

/**
 * Kiboko CI Build Runner
 * @author   Dan Cryer <dan@block8.co.uk>
 */
class Builder implements LoggerAwareInterface
{
    /**
     * @var DockerVirtualization
     */
    public $docker;
    ...
    /**
     * @var \Kiboko\Component\ContinuousIntegration\Plugin\Util\Executor
     */
    ...
    public function __construct(Build $build, LoggerInterface $logger = null)
    {
        ...
        $pluginFactory->addConfigFromFile(KIBOKO_CI_APP_DIR . "/pluginconfig.php");
        $this->pluginExecutor = new Plugin\Util\Executor($pluginFactory, $this->buildLogger);

        $executorClass = 'Kiboko\\Component\\ContinuousIntegration\\Helper\\UnixCommandExecutor';
        if (IS_WIN) {
            $executorClass = 'Kiboko\\Component\\ContinuousIntegration\\Helper\\WindowsCommandExecutor';
        }
    ...
    protected function setupBuild()
    {
        ...
        $this->docker = new DockerVirtualization(isset($this->config['php']['version']) ? $this->config['php']['version'] : 'latest');
        $this->docker->requirePackage(
            new AptitudePackage(
                isset($this->config['php']['packages']) ? array_keys($this->config['php']['packages']) : [],
                isset($this->config['php']['packages']) ? $this->config['php']['packages'] : [],
                isset($this->config['apt']['packages']) ? $this->config['apt']['packages'] : []
            )
        );
   ...
   /**
    ...
    private function buildPluginFactory(Build $build)
    {
        ...
        $pluginFactory->registerResource(
            ...
            null,
            'Kiboko\\Component\\ContinuousIntegration\\Builder'
        );
        ...
        $pluginFactory->registerResource(
            ...
            null,
            'Kiboko\\Component\\ContinuousIntegration\\Model\\Build'
        );
