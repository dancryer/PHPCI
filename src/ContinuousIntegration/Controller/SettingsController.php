...
class SettingsController extends Controller
{
    ...
    public function init()
    {
        ...
        $parser         = new Parser();
        $yaml           = file_get_contents(KIBOKO_CI_APP_CONFIG_FILE);
    ...
    public function index()
    {
        ...
        $this->view->configFile = KIBOKO_CI_APP_CONFIG_FILE;
    ...
    public function github()
    {
        ...
        if ($error) {
            $response->setHeader('Location', KIBOKO_CI_APP_URL . 'settings?saved=2');
        } else {
            $response->setHeader('Location', KIBOKO_CI_APP_URL . 'settings?saved=1');
        }
    ...
    public function githubCallback()
    {
        ...
        if (!is_null($code)) {
            ...
            if ($resp['success']) {
                ...
                $response = new b8\Http\Response\RedirectResponse();
                $response->setHeader('Location', KIBOKO_CI_APP_URL . 'settings?linked=1');
                return $response;
            }
        }

        $response = new b8\Http\Response\RedirectResponse();
        $response->setHeader('Location', KIBOKO_CI_APP_URL . 'settings?linked=2');
        return $response;
    }
    ...
    protected function storeSettings()
    {
        ...
        file_put_contents(KIBOKO_CI_APP_CONFIG_FILE, $yaml);
    ...
    protected function getGithubForm()
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL . 'settings/github');
    ...
    protected function getEmailForm($values = array())
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL . 'settings/email');
        $form->addField(new Form\Element\Csrf('csrf'));
    ...
    /**
     * Check if we can write the Kiboko CI config file.
     * @return bool
     */
    protected function canWriteConfig()
    {
        return is_writeable(KIBOKO_CI_APP_CONFIG_FILE);
    }
    ...
    protected function getBuildForm($values = array())
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL . 'settings/build');
    ...
    protected function getBasicForm($values = array())
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL . 'settings/basic');
    ...
    protected function getAuthenticationForm($values = array())
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL . 'settings/authentication');
        $form->addField(new Form\Element\Csrf('csrf'));
