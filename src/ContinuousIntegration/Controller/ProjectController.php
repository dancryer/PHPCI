...
use Kiboko\Component\ContinuousIntegration\Helper\SshKey;
...
* @subpackage   Web
*/
class ProjectController extends ContinuousIntegration\Controller
{
    /**
     * @var \Kiboko\Component\ContinuousIntegration\Store\ProjectStore
     */
    protected $projectStore;

    /**
     * @var \Kiboko\Component\ContinuousIntegration\Service\ProjectService
     */
    ...
    /**
     * @var \Kiboko\Component\ContinuousIntegration\Service\BuildService
     */
    ...
    */
    public function view($projectId)
    {
        ...
        if ($page > $pages) {
            ...
            $response->setHeader('Location', KIBOKO_CI_APP_URL.'project/view/'.$projectId);
            return $response;
        }
    ...
    */
    public function build($projectId, $branch = '')
    {
        ...
        $response = new b8\Http\Response\RedirectResponse();
        $response->setHeader('Location', KIBOKO_CI_APP_URL.'build/view/' . $build->getId());
        return $response;
    }
    ...
    */
    public function delete($projectId)
    {
        ...
        $response = new b8\Http\Response\RedirectResponse();
        $response->setHeader('Location', KIBOKO_CI_APP_URL);
        return $response;
    }
    ...
    */
    public function add()
    {
        ...
        if ($method != 'POST' || ($method == 'POST' && !$form->validate())) {
            ...
            $response = new b8\Http\Response\RedirectResponse();
            $response->setHeader('Location', KIBOKO_CI_APP_URL.'project/view/' . $project->getId());
            return $response;
        }
    }
    ...
    */
    protected function projectForm($values, $type = 'add')
    {
        $form = new Form();
        $form->setMethod('POST');
        $form->setAction(KIBOKO_CI_APP_URL.'project/' . $type);
