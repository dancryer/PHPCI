...
class Build extends BuildBase
{
    ...
    */
    ...
    {
        $pluginDir = KIBOKO_CI_APP_DIR . 'src/Plugin/';
        ...
        foreach ($dir as $item) {
            ...
            $className = 'Kiboko\\Component\\ContinuousIntegration\\Plugin\\'.$item->getBasename('.php');

            $reflectedPlugin = new \ReflectionClass($className);

            if (!$reflectedPlugin->implementsInterface('Kiboko\\Component\\ContinuousIntegration\\ZeroConfigPlugin')) {
                continue;
            }
    ...
    public function getBuildPath()
    {
        ...
        if (empty($this->currentBuildPath)) {
            ...
            $this->currentBuildPath = KIBOKO_CI_APP_BUILD_ROOT_DIR . $buildDirectory . DIRECTORY_SEPARATOR;
        }
